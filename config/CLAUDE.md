...
- claude-shared is scratch memory, not an archive to mine — reading stale/completed docs burns context and misleads (a shipped feature's old plan reads as "todo"). Default posture: **don't bulk-`grep`/read claude-shared; open the specific live file you need by name.**
- Two tiers. **warm** = the small live set you may read on demand (current petrichor plan, active `feedback.md`, `TODO.md`, open reports). **cold = `<shared-root>/permafrost/`** = everything else, hard off-limits: `Read`/`grep`/`find` are denied in settings, so reading needs an explicit thaw. You *can* `mv` files in (freeze) but not read them back.
- Keep warm thin (**eviction**): completed `TODO.md` items don't linger — promote the keepers (issue / repo docs), prune the line, freeze the rest. **Never raw-delete** (claude-shared isn't git — deletion is unrecoverable); freeze into `permafrost/` instead.
- `/permafrost` runs the sweep (propose freeze / promote candidates → confirm → execute) and `thaw`; suggest it at cleanup checkpoints. `almanac` proposes stale candidates into the same store.
