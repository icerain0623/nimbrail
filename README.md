...
| `windshear` | OWASP coverage over the standing codebase, not the diff: enumerate the attack surface (entry points, authz decisions, raw query sites, egress, sinks, config, deps), then mark every OWASP category 対策あり (with the `file:line` where it is *enforced*) / 不在 / 該当なし. The empty cell is the finding — a control missing everywhere never shows up in a diff, so `/security-review` cannot see it. Read-only, proposes |
