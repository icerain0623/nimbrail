...
| `legend` | conventions for a document someone *executes* — runbook, deploy procedure, handover: top-to-bottom order, one paste per code block with its expected result and its branch, recovery in an appendix, no tables or checkboxes. Ships `selfcheck.sh` for the mechanical pass at the end |
