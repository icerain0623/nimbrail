...
Before Phase 0, ask **one** question: how far should this go? The answer sets interview depth, which sections to cover, and the Done bar. Settle the **spec language** in the same exchange (not a separate round-trip): `ja` (default) / `en` / `ja+en` — dual means one **canonical** language (listed first) plus a translation **rendered at Done**, never authored in parallel. The interview and the `NN-topic.md` working files always stay in the user's language; the setting governs the deliverable only. Recommend `ja` for personal/domestic work, `en` or `ja+en` when the repo is headed for OSS or international collaborators. Record level and language in the `00-overview.md` header.
...
- **L3 — requirements definition / 要件定義**: full coverage driven by `requirements-jp.md` (sibling file) — its section set, each with start/done/review criteria; the spec body follows the chosen **spec language** (default ja). Heavy; choose only when a complete spec is wanted. Scope stops at 基本設計（外部設計）; 詳細設計・実装・テスト・運用 are out of scope (詳細設計 belongs to `squall`, 実装 to the build phase).
...
All planning lives **outside the repo**, in the Obsidian-readable shared dir — so it never clutters the codebase or enters git, and the user can read/edit it in Obsidian. The shared root defaults to `~/Documents/claude-shared`, with per-project overrides in `~/.claude/shared-dirs.json` (resolution rule: global CLAUDE.md → Handoff files); everything below sits under the resolved root. `<project>` = basename of `<project-root>`: the git repo toplevel (`git rev-parse --show-toplevel`) if inside one, otherwise the working directory you are planning in — never a parent/container dir. If the cwd is a parent that holds several projects (e.g. you were launched from `~/Developers`), establish the project directory first. Create the dir + files if missing.
