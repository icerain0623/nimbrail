...
`<shared-root>/<project>/forecast-checklist.md` — Obsidian-readable, checkbox-driven, **never committed** (a run document, not a design record). Re-running regenerates it from the current spec (overwrite); past runs' results live in the release's git history, not here.
...
- **Every line traces to an ID**, and expected results are quoted or derived from the acceptance criteria (EARS sentences) — never invent a new bar. An expectation you cannot write is a **spec hole**: report it as a candidate to route back to `petrichor`, don't silently fill it in.
