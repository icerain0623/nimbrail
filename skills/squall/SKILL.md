...
- Design prose takes its language from the petrichor spec header, else the repo's existing docs, and records it as `docs_lang` in `project.md`. Code artifacts (Lint config, OpenAPI, IaC) follow the repo's own conventions instead.
