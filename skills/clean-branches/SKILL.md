...
2. Refresh refs: `git fetch --prune origin`. If `origin` is unreachable, skip it and say so (then judge against local `<main>` and warn it may be stale).
...
5. Show ONE list — each local branch to delete, and beside it the remote counterpart that will also go (when it exists and is merged) — with the exact commands. Wait for a single explicit confirmation.
