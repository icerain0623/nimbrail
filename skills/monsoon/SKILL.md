...
2. A build is mid-flight (claude-shared `tasks.md` exists) with **3+ unblocked todo tasks**, and the current ask isn't an interactive express-lane change → suggest `/downpour` (wave burn-down of the ledger). Propose only — downpour never auto-starts.
3. Uncommitted changes → run `check` (default tier). If it passes, commit using the built-in commit behavior (follow the CLAUDE.md Git rules — autonomous commit is allowed); if it fails, summarize the failures and stop.
