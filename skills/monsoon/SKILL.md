...
10. This session wrote a handoff document to the shared root (a runbook, work instructions, a handover) → run `legend`'s `selfcheck.sh` over it and report the numbers. The measurement only: rewriting the user's prose unasked is not this router's call, and `legend` is slash-only precisely so its rules never ride along in context — quoting them here would undo that.
11. Nothing pending here → **run** `synoptic` (full, unnarrowed), don't offer it: this router only sees the current repo, so a sibling stopped on your verification is invisible from it, and this step is the moment that question actually arises. Running it is also what keeps `status.md` current — invoked on its own it fires a fraction as often as this router does. `sunbreak` on explicit request.
