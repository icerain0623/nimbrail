...
0. **A new piece of work is being requested** (an actual new feature/change — not "look at the state and tell me what's next"): triage by size before anything else.
   - **Trivial / small / well-understood → express lane.** Skip the planning stations (petrichor/squall); implement in the normal loop, then `check` → confirm real behavior → commit.
   - **Substantial / underspecified → re-enter the rail at `petrichor`** (plan → `squall` for design + config, then build in the normal loop).
   If instead the ask is "do the next sensible thing" given current state, fall through to the state-based steps below. When a `tasks.md` exists and a build is mid-flight, that is the next unblocked task (dependencies marked done in the ledger); name it and its completion condition.
...
3. A build is mid-flight (`tasks.md` exists) with 3+ unblocked todo tasks, and the current ask isn't an interactive express-lane change → suggest `/downpour` (wave burn-down of the ledger). Placed after step 2 on purpose: downpour's preflight requires a clean tree, so suggesting it on a dirty tree would bounce.
...
5. On a feature branch, everything committed, checks pass → `private-scan` the outgoing range, then offer to push / open a PR. This is the rail's only "now we publish" decision, so it is where the scan belongs rather than left to fire on its own.
...
9. `findings.md` has unchecked lines above its `## 対応済み` heading that none of the steps above already covers → surface them, most severe 分類 first, and ask which to take: each was logged precisely because it was out of scope when found, so picking it up is the user's call. A project whose own CLAUDE.md routes findings to its issue tracker instead: read its open issues in place of the file.
