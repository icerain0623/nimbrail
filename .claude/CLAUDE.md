...
- Run `lint-skills.sh` by hand after a rename or a delete, which no edit signals. Checks [8] and [9] budget the two always-loaded surfaces — `config/CLAUDE.md`, and the name+description of every model-invocable skill — so adding to either means trimming elsewhere or raising the budget on purpose. [8]'s 6000 chars is **this repo's own limit**; the platform targets 200 lines per file. [9] counts model-invocable skills only.
