...
- After adding or renaming a skill, or editing skill cross-references or shared-dir paths, run `bash lint-skills.sh`. Its check [8] budgets `config/CLAUDE.md`, which is loaded on every request — adding a rule there means either trimming another or raising the budget on purpose.
...
- Sandbox carve-out: git ops that **rewrite working-tree files under `config/`** (checkout/merge/reset across branches differing there) fail in-sandbox with `Operation not permitted` — the harness-injected `.git/config` protection is missing its `.git/` prefix and hits this repo's `config/` dir. Run those ops with the sandbox disabled; Edit-tool edits and commits are unaffected. Not fixable from this repo's settings.
- Branch switches (`checkout` / `switch` / `merge` / `reset`) therefore run with the sandbox disabled, and `git status` afterwards: in-sandbox they fail per-file under `config/` **while still exiting 0**, leaving a tree where some files moved and some didn't.
