...
| `windshear` | diff ではなくコードベース全体に対する OWASP カバレッジ。まず攻撃面を列挙し（エントリポイント、認可判断の場所、生クエリ、外向き通信、シンク、設定、供給網、ログ、失敗経路）、そのうえで OWASP Top 10:2025 の各カテゴリを（版とカテゴリごとの probe は `catalogue.md` に固定） 対策あり（*強制されている* `file:line` 付き）/ 不在 / 該当なし で埋める。空のセルが所見そのもの — どこにも無い対策は diff に現れないので `/security-review` からは見えない。読み取りのみで提案する |
| `forecast` | 仕様書からリリース前のシナリオテスト チェックリストを生成する（機能 ID へのカバレッジ追跡付き） |
| `legend` | 書き上がった文書に対する推敲パス。第1層（ファイルなら何でも）は AI っぽさの表層マークアップを落とす — 飾りの強調（このリポジトリ自身に `lint-skills.sh` が課しているのと同じ 1.5/1000字 の密度で測る）、非表形式データの表組み、入れ子の箇条書き、区切り線、絵文字や ⚠ ✅ のような記号の印。日本語の文章にはさらに常套句・「ではなく」の反復・文長の変動係数・文をつなぐダッシュ・Claude の口癖（効く、黙って など）を数える（閾値は coji/natural-japanese のコーパス校正から）。二つの層のあいだに、判断で行う読み返しパスがある。まず構成 — 追記節や調査ラウンドを読み手の問いの順に畳み込み、異常時の話は付録へ、書き手が受けた指示の記録と購入済みの商品の通販リンクは外し、手順と知見が混ざっていれば分け方を提案する — 次に見出しと段落先頭文だけのスケルトン、結論が先に来ているか、用語は初出で説明されているか、日本語なら文の読解負荷のカタログ（`readability.md`）と、長い文・漢字の連続・「の」の連鎖・二重否定を正規表現で指さす検出。直すのは読み手の得になる箇所だけで、規則を全箇所に一律に当てるとそれ自体が次の AI っぽさになる。第2層（`runbook.md`。人が*実行する*文書 — 手順書・デプロイ手順・引き継ぎ — のときだけ読む）は、上から順に実行できる並び、1コードブロック＝1回の貼り付けとその期待結果・分岐、異常時対応は末尾の付録。**書く前ではなく書いた後**に走らせるのは意図的で、文体制約を生成中に抱えると精度を払うため。`selfcheck.sh` 同梱 |
