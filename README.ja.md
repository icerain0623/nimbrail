...
| `windshear` | diff ではなくコードベース全体に対する OWASP カバレッジ。まず攻撃面を列挙し（エントリポイント、認可判断の場所、生クエリ、外向き通信、シンク、設定、依存）、そのうえで各 OWASP カテゴリを 対策あり（*強制されている* `file:line` 付き）/ 不在 / 該当なし で埋める。空のセルが所見そのもの — どこにも無い対策は diff に現れないので `/security-review` からは見えない。読み取りのみで提案する |
